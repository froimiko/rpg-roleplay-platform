...
## §6 v1 UX 勘误(用户反馈,2026-07-06)

- 命名去中二:页面就叫 RATH(去「搖光观测台」后缀);观测时间线→**日志**;搖光单元→角色动态;观测钟→世界时间。
- 运行=**开关**(Toggle,on=running/off=paused),保留「推进一步」手动按钮;「今日预算」改为人话说明(自动推进频率+今日次数/上限)。
- **引导重设计**:废弃 rath_experiments.directive 全局状态列(语义=「保存的某个状态」,反直觉)。新语义=**插入日志的节点事件**(rath_events kind=directive):从插入时间点开始引导其后的演化,最新一条生效,历史全部留在日志可查。
- 时间语义显式化:时间流速 N× = 现实1分钟≈世界N分钟;推进一步≈世界1小时。
