...
- `platform_app/api/` — 平台 REST 路由。`frontend_routes.py` — 静态前端投喂。`scripts/`(原 `scripts.py` 1728 行已按资源族包化:`listing`/`chapters`/`cards`/`worldbook_canon`/`media`/`imports`/`library`/`overrides`/`review` 子模块共享 `_shared.router`,`__init__` 薄门面 re-export)供 `/api/scripts*`、`/api/uploads/*`。
