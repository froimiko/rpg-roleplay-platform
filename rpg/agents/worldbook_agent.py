...
def consult(
    ...
    *,
    ...
    if result.confidence == 0.0:
        # 退化兜底:有 script_id 却 anchor/phase_digest/chapter_facts/worldbook 全空
        # (典型=未拆书剧本)→ consult 空手而归,下游用空世界书静默合成。记一行 info,让
        # 「未拆书剧本被静默兜底」可观测(与 get_progress_window 的 fallback 同回合出现 = 强信号)。
        log.info(
            "[worldbook.consult] 退化空结果(未拆书/无匹配):save=%s script=%s query=%r",
            save_id, script_id, (query or "")[:40],
        )
    return result
